...
export class ClauseMatchingView extends React.Component {
    ...
    sortClasses(){
        for (var i = 0; i < this.state.truth.length; i++){
            ...
            for (var j = 0; j < predictions.length; j++) {
                const related_clause = this.state.data.filter(clause => clause.id === predictions[j].clause);
    ...
    getPredictions(agbid, methodid){
        fetch('http://localhost:5000/predictions/clause/'+ agbid + '/' + methodid, {
            method : 'GET',
    ...
    getClauses(id){
        fetch('http://localhost:5000/clausesFromAGB/' + id, {
            method : 'GET',
            header : 'Access-Control-Allow-Origin'
        }).then(res => res.json()
        ).then(response => {
            if(this.state.truthLoaded === false){
                ...
                this.getClauses(this.state.agbid)
            }else{
                this.setState({
                    data: [...response]
                });
    ...
    setTrueState(){
        fetch("http://localhost:5000/setTrueState/clause", {
            method : 'PUT',
